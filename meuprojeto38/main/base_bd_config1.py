...
def conecta_no_banco_de_dados():
    ...
    cursor = cnx.cursor()
    cursor.execute('SELECT COUNT(*) FROM information_schema.SCHEMATA WHERE SCHEMA_NAME = "taranbanco";')
    ...
    if num_results > 0:
        print('O banco de dados taranbanco existe e esta pronto para uso.')
    else:
        ...
        # Criar o banco de dados taranbanco
        cursor = cnx.cursor()
        cursor.execute('CREATE DATABASE taranbanco;')
        cnx.commit()

    # Conectar-se ao banco de dados taranbanco recém-criado
        cnx = mysql.connector.connect(
        host='127.0.0.1',
        user='root',
        password='',
        database='taranbanco'  # Especificar o banco de dados
        )
